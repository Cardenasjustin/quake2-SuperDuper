...
void Cmd_SelectClass_f(edict_t* ent, int class_id)
{
	ent->client->shows_class_menu = false;

	ent->client->pers.inventory.fill(0); // Clear

	switch (class_id)
	{
	case 1:
		ent->client->selected_class = 1;
		ent->client->pers.inventory[FindItem("Chaingun")->id] = 1;
		ent->client->pers.inventory[FindItem("Bullets")->id] = 200;
		ent->client->newweapon = FindItem("Chaingun");
		break;
	case 2:
		ent->client->selected_class = 2;
		ent->client->pers.inventory[FindItem("Rocket Launcher")->id] = 1;
		ent->client->pers.inventory[FindItem("Rockets")->id] = 40;
		ent->client->newweapon = FindItem("Rocket Launcher");
		break;
	case 3:
		ent->client->selected_class = 3;
		ent->client->pers.inventory[FindItem("Railgun")->id] = 1;
		ent->client->pers.inventory[FindItem("Slugs")->id] = 20;
		ent->client->newweapon = FindItem("Railgun");
		break;
	case 4:
		ent->client->selected_class = 4;
		ent->client->pers.inventory[FindItem("Shotgun")->id] = 1;
		ent->client->pers.inventory[FindItem("Shells")->id] = 50;
		ent->client->newweapon = FindItem("Shotgun");
		break;
	case 5:
		ent->client->selected_class = 5;
		ent->client->pers.inventory[FindItem("HyperBlaster")->id] = 1;
		ent->client->pers.inventory[FindItem("Cells")->id] = 100;
		ent->client->newweapon = FindItem("HyperBlaster");
		break;
	}
	ent->client->has_selected_class = true;
	ent->client->shows_class_menu = false;

	// Clear the center message
	gi.LocCenter_Print(ent, "");
	gi.LocCenter_Print(ent, "Press [F] to activate your ability!");

}
void ActivateClassAbility(edict_t* ent)
{
	if (ent->client->selected_class && level.time < ent->client->ability_cooldown_end_time)
	{
		gi.LocCenter_Print(ent, "Ability is on cooldown!");
		return;
	}

	switch (ent->client->selected_class)
	{
	case 1: // Soldier - Unlimited ammo
		ent->client->ammo_override_end_time = level.time + 10_sec;
		gi.LocCenter_Print(ent, "Unlimited ammo for 10 seconds!");
		break;

	case 2: // Heavy - 
	{
		gi.LocCenter_Print(ent, "NUKE ACTIVATED!");
		...
		for (int i = 1; i < globals.num_edicts; ++i)
		{
			...
			{
				vec3_t diff = {
					target->s.origin[0] - ent->s.origin[0],
					target->s.origin[1] - ent->s.origin[1],
				...
				T_Damage(target, ent, ent, vec3_origin, target->s.origin, vec3_origin, 9999, 0, DAMAGE_NO_KNOCKBACK, MOD_UNKNOWN);
			}
		}

		break;
	}
	case 3: // Sniper - Dash & melee spam
	{
		// Allow re-use during active dash/melee window
		if (ent->client->melee_end_time > level.time)
		{
			// Dash again
			vec3_t forward;
			...
			gi.LocCenter_Print(ent, "Dash boosted!");
			return;
		}

		// If cooldown is still active, deny usage
		if (level.time < ent->client->ability_cooldown_end_time)
		{
			gi.LocCenter_Print(ent, "Dash ability is on cooldown!");
			return;
		}

		// Start dash + melee mode
		vec3_t forward;
		AngleVectors(ent->client->v_angle, forward, NULL, NULL);
		...
		// Save current weapon
		ent->client->previous_weapon = ent->client->pers.weapon;

		// Give Chainfist
		gitem_t* melee = FindItem("Chainfist");
		if (melee) {
			ent->client->pers.inventory[melee->id] = 1;
			ent->client->pers.weapon = melee;
			ent->client->newweapon = melee;
			ChangeWeapon(ent);
		}

		ent->client->melee_end_time = level.time + 2_sec; // spam window

		gi.LocCenter_Print(ent, "Dash activated!");
		break;
	}

	case 4: // Support - Invincibility
	{
		ent->client->invincible_until = level.time + 10_sec;
		gi.LocCenter_Print(ent, "Invincibility activated!");
		break;
	}

	case 5: // Blaster Master - Thorns reflect
	{
		ent->client->thorns_end_time = level.time + 10_sec;
		gi.LocCenter_Print(ent, "Thorns activated!");
		break;
	}


	default:
		gi.LocCenter_Print(ent, "No ability for this class.");
		return;
	}
...
void ClientCommand(edict_t *ent)
{
	...
	{
		// Prevent reopening menu if already picked
		if (ent->client->has_selected_class)
		{
			gi.LocCenter_Print(ent, "You have already selected a class.");
			return;
		}

		ent->client->shows_class_menu = true;

		gi.LocCenter_Print(ent,
			"=== CLASS SELECTION ===\n"
			"[1] MAX AMMO - Chaingun\n"
			"[2] NUKE - Rocket Launcher\n"
			"[3] DASH - Railgun\n"
			"[4] INVINCIBLE - Shotgun\n"
			"[5] THORNS - HyperBlaster\n");
		return;
	}
	if (_stricmp(cmd, "useability") == 0)
	{
		ActivateClassAbility(ent);
		return;
	}

	// Handle selection keys
	if (ent->client->shows_class_menu)
	{
		if (_stricmp(cmd, "1") == 0) { Cmd_SelectClass_f(ent, 1); return; }
		if (_stricmp(cmd, "2") == 0) { Cmd_SelectClass_f(ent, 2); return; }
		if (_stricmp(cmd, "3") == 0) { Cmd_SelectClass_f(ent, 3); return; }
		if (_stricmp(cmd, "4") == 0) { Cmd_SelectClass_f(ent, 4); return; }
		if (_stricmp(cmd, "5") == 0) { Cmd_SelectClass_f(ent, 5); return; }
	}
