			  ...
			  const vec3_t &normal, int damage, int knockback, damageflags_t dflags, mod_t mod)
{
	...
	if (!targ->takedamage)
		return;
	// Support class invincibility
	if (targ->client && targ->client->invincible_until > level.time)
	{
		// Optional: play a sound
		gi.sound(targ, CHAN_AUTO, gi.soundindex("items/protect4.wav"), 1, ATTN_NORM, 0);
		return; // cancel all damage
	}
	// Thorns: Reflect fatal damage if active
	if (targ->client && targ->client->thorns_end_time > level.time)
	{
		if (attacker && attacker != targ && attacker->takedamage && attacker->health > 0)
		{
			// Optional: play effect
			T_Damage(attacker, targ, targ, vec3_origin, attacker->s.origin, vec3_origin, 9999, 0, DAMAGE_NO_KNOCKBACK, MOD_UNKNOWN);
			gi.LocCenter_Print(targ, "Thorns triggered!");
		}
	}
