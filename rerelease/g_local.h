...
extern int g_total_monsters_killed;
...
struct gclient_t
{
	...
	gtime_t next_item_spawn_time; // custom field for timed item spawning
	bool shows_class_menu = false;
	bool has_selected_class = false;
	int selected_class = 0; // 0 = none, 1 = Soldier, 2 = Heavy, etc.
	gtime_t ammo_override_end_time;
	gtime_t ability_cooldown_end_time = 0_sec;
	gtime_t invincible_until = 0_sec;
	gtime_t lockon_until = 0_sec;
	gtime_t melee_end_time = 0_sec;
	gitem_t* previous_weapon = nullptr;
	gtime_t thorns_end_time = 0_sec;
...
inline void VecClear(vec3_t v)
{
	v[0] = v[1] = v[2] = 0;
}

inline void VecSubtract(const vec3_t a, const vec3_t b, vec3_t out)
{
	out[0] = a[0] - b[0];
	out[1] = a[1] - b[1];
	out[2] = a[2] - b[2];
...
inline float VecLength(const vec3_t v)
{
