...
int g_monster_kill_count = 0;
static float g_monster_health_multiplier = 1.1f;
...
{
	g_monster_health_multiplier *= 1.1f;
	level.killed_monsters++;
	g_monster_kill_count++;
...
// Insert here, before monster_start()
void ApplyMonsterHealthBonus(edict_t* self)
{
	...
	int bonus = (int)(self->monsterinfo.base_health * 0.1f);
	self->health += bonus;
...
bool monster_start(edict_t *self)
{
	...
	// Apply global health multiplier
	self->health = (int)(self->health * g_monster_health_multiplier);
	self->max_health = self->health;

	// [Paril-KEX] co-op health scale
	G_Monster_ScaleCoopHealth(self);

	ApplyMonsterHealthBonus(self);
...
void monster_start_go(edict_t *self)
{
	int health_bonus = self->health - self->monsterinfo.base_health;
