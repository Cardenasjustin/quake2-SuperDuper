...
#define VectorNormalize(v) _VectorNormalize(v)
...
void fire_blaster(edict_t *self, const vec3_t &start, const vec3_t &dir, int damage, int speed, effects_t effect, mod_t mod)
{
	...
	char msg[128];
	gi.LocCenter_Print(&g_edicts[1], msg);
...
void Blaster_Fire(edict_t* ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	// Center shot
	fire_blaster(ent, start, forward, damage, 1000, (effects_t)effect, mod);

	// Left spread
	vec3_t leftDir;
	VectorMA(forward, -0.9f, right, leftDir);
	VectorNormalize(leftDir);
	fire_blaster(ent, start, leftDir, damage, 1000, (effects_t)effect, mod);

	// Right spread
	vec3_t rightDir;
	VectorMA(forward, 0.9f, right, rightDir);
	VectorNormalize(rightDir);
	fire_blaster(ent, start, rightDir, damage, 1000, (effects_t)effect, mod);
}
