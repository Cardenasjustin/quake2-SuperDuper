...
void ClientThink(edict_t* ent, usercmd_t* ucmd)
{
	...
	if (level.time >= ent->client->next_item_spawn_time) {
		const int num_items = 10;

		item_id_t possible_items[] = {
			IT_AMMO_BULLETS,
			IT_AMMO_SHELLS,
			IT_AMMO_CELLS,
			IT_AMMO_ROCKETS,
			IT_AMMO_GRENADES,
			IT_AMMO_SLUGS,
			IT_AMMO_MAGSLUG,       // RAFAEL
			IT_AMMO_FLECHETTES,    // ROGUE
			IT_ARMOR_SHARD,
			IT_ARMOR_COMBAT,
			IT_ARMOR_BODY
		};

		const int total_items = sizeof(possible_items) / sizeof(possible_items[0]);

		for (int i = 0; i < num_items; ++i) {
			vec3_t spawn_pos = {
				ent->s.origin[0] + crandom() * 200.0f,
				ent->s.origin[1] + crandom() * 200.0f,
				ent->s.origin[2] + 20.0f
			};

			item_id_t item_id = possible_items[rand() % total_items];
			gitem_t* item = GetItemByIndex(item_id);
			if (!item) continue;

			edict_t* item_ent = G_Spawn();
			if (!item_ent) continue;

			SpawnItem(item_ent, item);
			item_ent->s.origin[0] = spawn_pos[0];
			item_ent->s.origin[1] = spawn_pos[1];
			item_ent->s.origin[2] = spawn_pos[2];
			gi.linkentity(item_ent);
		}
		ent->client->next_item_spawn_time = level.time + 20_sec;

		if (ent->client->selected_class > 0 && // ensure a class is selected
			level.time >= ent->client->ability_cooldown_end_time &&
			ent->client->ability_cooldown_end_time > 0_sec) // avoid false triggers
		{
			gi.LocCenter_Print(ent, "Your ability is ready! Press [F]");
			ent->client->ability_cooldown_end_time = 0_sec; // clear to avoid repeating
		}

		if (ent->monsterinfo.frozen_until > level.time) {
			VecClear(ent->velocity);
			return;
		}

		if (ent->client->melee_end_time > 0_sec &&
			level.time >= ent->client->melee_end_time)
		{
			// Restore previous weapon
			if (ent->client->previous_weapon) {
				ent->client->pers.weapon = ent->client->previous_weapon;
				ent->client->newweapon = ent->client->previous_weapon;
				ChangeWeapon(ent);
			}
			...
			if (melee) {
				ent->client->pers.inventory[melee->id] = 0;
			}

			ent->client->melee_end_time = 0_sec;
			ent->client->previous_weapon = nullptr;
