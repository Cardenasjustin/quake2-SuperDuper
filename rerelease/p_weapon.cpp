...
#include "g_local.h"
#include "m_player.h"
#define Com_ErrorFmt(fmt, ...) gi.error(fmt, ##__VA_ARGS__)
...
void NoAmmoWeaponChange(edict_t *ent, bool sound)
{
	...
	for (size_t i = 0; i < q_countof(no_ammo_order); i++)
	{
		...
		if (!item)
			// Com_Error(ERR_DROP, "Invalid no ammo weapon switch weapon %d\n", (int32_t)no_ammo_order[i]);
...
{
	if (ent->client && ent->client->selected_class == 1 && level.time < ent->client->ammo_override_end_time) {
		return; // Soldier class with unlimited ammo active
	}
	if (ent->client && ent->client->selected_class == 1 && level.time < ent->client->ammo_override_end_time) {
		return; // Soldier class with unlimited ammo active
	}
...
void Blaster_Fire(edict_t* ent, const vec3_t& g_offset, int damage, bool hyper, effects_t effect)
{
	...
	mod_t mod = hyper ? MOD_HYPERBLASTER : MOD_BLASTER;

	vec3_t forward, right, up, spread_dir;
	AngleVectors(ent->client->v_angle, forward, right, up);

	// Center shot
	fire_blaster(ent, start, forward, damage, speed, effect, mod);

	// Slight right shot
	VectorMA(forward, 0.15f, right, spread_dir);
	fire_blaster(ent, start, spread_dir, damage, speed, effect, mod);
